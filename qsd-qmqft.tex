...
This framework preserves all predictions of QM and QFT while providing a physical mechanism for collapse, emission, and entanglement. Wavefunctions, superpositions, and fields are real—but only within the CI, where coherence is supported. Collapse is not triggered by observation, but by causal exhaustion of the interval. Importantly, collapse outcomes are \textit{deterministic with respect to their hidden inputs}: given the full coherence configuration of a CI, the CB resolves uniquely. Yet those inputs—residual CPA fields, scalar recovery availability, inherited phase geometry—are causally inaccessible to external observers. The apparent randomness of quantum outcomes thus arises not from indeterminacy, but from epistemic limitation. The Born Rule is preserved as a statistical projection of inaccessible determinism.

Entanglement is not instantaneous linkage, but resonance tuning: structures formed within the same coherence envelope remain phase-compatible and may respond to serialized emissions if their tick pacing remains aligned. These emissions do not enforce collapse in the partner structure, but may stimulate a re-lock if coherence conditions permit.
...
\begin{itemize}
    \item  OpenAI’s ChatGPT (version GPT-4o, 2025)  was used for editorial and formatting processes.
...
The Quantum Substrate Dynamics (QSD) framework yields a deterministic structure for collapse behavior and quantized evolution through physically grounded constraints. Three primary structural results are derived:

\subsection{Collapse Boundary (CB)}

Collapse is not a stochastic measurement event but a structural failure in the substrate when internal coherence exceeds the local offload capacity. This failure defines the \emph{Collapse Boundary (CB)}: a critical surface where a structure can no longer be supported by the substrate's tension envelope.

The collapse condition is expressed by a threshold offload energy:
\begin{equation}
E \geq \frac{c_t^4}{G} \cdot L_{\text{coh}},
\end{equation}
where \( c_t \) is the transverse coherence propagation speed, \( G \) is the curvature compliance constant (gravitational constant), and \( L_{\text{coh}} \) is the local coherence support length. Exceeding this threshold necessitates an irreversible structural emission event.

\subsection{Causality Interval (CI)}

Quantum evolution occurs within discrete spacetime envelopes, each bounded by a physically determined coherence window. This window is defined as a \emph{Causality Interval (CI)}, and its temporal extent is governed by the scalar recovery speed \( c_s \) of the substrate. The interval duration is given by:
\begin{equation}
\tau = \frac{L_{\text{coh}}}{c_s},
\end{equation}
indicating the minimum time required for the substrate to reset its coherence state and support the next phase of evolution.

\subsection{Quantum Emission Opportunity (QEO)}

At the termination of each CI, the system encounters a deterministic offload point called the \emph{Quantum Emission Opportunity (QEO)}. Emission occurs not probabilistically, but structurally, as a serialized projection of the internal waveform’s geometry.

Each QEO represents a substrate-mandated decision boundary where:
\begin{itemize}
    \item Emission is permitted only if the structure is at or beyond the collapse threshold.
    \item If not, coherence persists through the next CI.
\end{itemize}
This mechanism reinterprets quantum behavior as a series of structurally enforced emission gates, rather than wavefunction collapse triggered by external measurement.  See Figure \ref{fig:cicbqeo}.

\subsection{Derived Role of Planck Constants}

Planck constants are reinterpreted as structural expressions of substrate behavior:

\begin{itemize}
    \item Planck energy \( E_P \) defines the maximum offload threshold of the substrate:
    \begin{equation}
    E_P = \frac{c_t^4}{G} \cdot L_{\text{coh}}.
    \end{equation}
    
    \item Planck time \( t_P \) is the minimum interval for causal reset:
    \begin{equation}
    t_P = \frac{L_{\text{coh}}}{c_s}.
    \end{equation}
    
    \item Planck’s constant \( \hbar \) arises from the ratio of transverse to scalar propagation modes scaled by geometry:
    \begin{equation}
    \hbar = \frac{c_t^4}{c_s} \cdot \frac{L_{\text{coh}}^2}{G}.
    \end{equation}
\end{itemize}

These expressions ground the Planck units in physically interpretable substrate dynamics, rather than abstract limits.

\subsection{Implications}

This framework produces several key implications:

\begin{enumerate}
    \item Collapse is a causal, localized emission event, not a measurement-induced discontinuity.
    \item Quantization and emission timing are determined by internal structural pacing, not probabilistic sampling.
    \item The substrate imposes natural intervals for information serialization, consistent with observed quantum evolution.
    \item Observable outcomes correspond to the serialized geometric projection of coherent structures at each QEO.
\end{enumerate}

These results unify quantum collapse, time pacing, and Planck-scale thresholds under a single structural model, providing a causal framework for coherence, quantization, and measurement phenomena.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Collapse Statistics as Substrate Constraint Projection: A Causal Interpretation of the Born Rule}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

In the QSD framework, collapse is not an indeterminate or probabilistic process from the substrate’s perspective. Each re-lock event at the end of a Causality Interval (CI) is governed by the local configuration of structural constraints—specifically: CPA memory geometry, scalar recovery availability, and waveform compatibility within the finite coherence envelope \( L_{\text{coh}}^3 \).

However, all of these conditions exist entirely within the substrate and are causally isolated during the CI. External observers have no access to these internal coherence dynamics while they evolve. From the observer's standpoint, the eventual collapse outcome appears probabilistic because the substrate's internal configuration at the moment of resolution is epistemically inaccessible.

\begin{quote}
\textit{What appears as quantum randomness is not ontological—but rather a projection of causal constraint geometry filtered through structural ignorance.}
\end{quote}

This offers a reinterpretation of the Born Rule. Rather than treating outcome probabilities as fundamental, QSD treats them as the \textbf{statistical shadow of inaccessible coherence logic} within \( L_{\text{coh}}^3 \). The apparent probability of a quantum outcome is a measure of how many causal re-lock pathways exist across the substrate's directional constraint map—how many structural routes are viable given the CPA field's state, the available scalar bandwidth, and the waveform phase geometry.

\begin{equation}
P(\vec{r}, \theta, \phi) = f_{\text{CPA}}(\vec{r}, \theta, \phi) \cdot f_{\text{scalar}}(\vec{r}, \theta, \phi) \cdot f_{\text{waveform}}(\vec{r}, \theta, \phi)
\label{eq:constraint_projection}
\end{equation}


This defines a probability amplitude not as a wavefunction amplitude squared, but as a \textit{directionally resolved causal compatibility} function—a field of constraint gradients over which collapse may occur.

\begin{center}
\fbox{%
\begin{minipage}{0.94\textwidth}
\textbf{QSD Interpretation of the Born Rule.} The Born Rule arises from causal ignorance. Collapse outcomes appear probabilistic because observers cannot access the structural constraints evolving within the causality interval. The probability distribution represents the projected geometry of viable re-lock directions inside the coherence volume \( L_{\text{coh}}^3 \), as structurally expressed in Eq.~\eqref{eq:constraint_projection}. QSD preserves statistical predictions while grounding them in substrate determinism.
\end{minipage}
}
\end{center}


This view resolves the tension between quantum randomness and causal structure. It reframes wavefunction collapse not as the breaking of symmetry, but as the emergence of structure from within a causally constrained and memory-saturated coherence field.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Collapse Determinacy and Apparent Randomness}
\label{subsec:determinacy}

In Quantum Substrate Dynamics, collapse is not fundamentally random. Each outcome at a Collapse Boundary (CB) is uniquely determined by the complete coherence configuration of the preceding Causality Interval (CI). Once the inputs are given, the substrate enforces a single re-lock outcome with no ambiguity. Collapse is therefore strictly deterministic at the substrate level.

The key distinction lies in the inputs themselves. Each CI inherits residual CPA memory, scalar recovery availability, and phase geometry accumulated since the origin of the system. These inherited conditions define the true state space of the CI. However, they are causally inaccessible to any observer, because the internal evolution of a CI cannot be probed without becoming part of the same unresolved frame. As a result, no experiment can recover the full coherence configuration that determines collapse. 

From the observer’s standpoint, collapse outcomes therefore appear probabilistic. The Born Rule is reinterpreted in this framework as the statistical shadow of inaccessible determinism: the probability distribution reflects how many viable re-lock pathways exist in the substrate’s constraint geometry, not an ontological indeterminacy in nature itself. Randomness is not fundamental—it is epistemic, arising from structural ignorance of hidden CI inputs.

\begin{center}
\fbox{%
\begin{minipage}{0.94\textwidth}
\textbf{Collapse Determinacy Principle (QSD).}  
Collapse outcomes are deterministically fixed by the full coherence configuration of a Causality Interval.  
Apparent randomness arises because the inputs to that determinism are causally inaccessible.  
The Born Rule preserves the statistical projection of this inaccessibility while grounding it in substrate-level determinism.
\end{minipage}
}
\end{center}

A final implication follows. For reality to manifest as change, collapse outcomes must differ across intervals. If the CB always re-locked into the identical configuration, time would advance in the substrate but no novelty would appear in the observed world—reality would be frozen, as in the children’s game of \textit{freeze tag}. Thus, apparent randomness is not incidental; it is the structural condition that allows motion, history, and experience to emerge from an otherwise determinate substrate.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
A central implication of this framework is that collapse is \textit{deterministic} at the substrate level: given the complete coherence configuration of a CI, the CB resolves uniquely. Yet the inputs to that determinism—residual CPA memory, scalar recovery availability, and inherited phase geometry—are causally inaccessible while the CI evolves. For embedded observers, collapse outcomes therefore appear probabilistic. The Born Rule emerges as the statistical projection of inaccessible determinism. Apparent randomness is not fundamental—it is the condition that allows reality to manifest as change. If every collapse re-locked identically, the world would be frozen, advancing in substrate ticks but with no observable novelty. Reality itself depends on differing outcomes across intervals.

This framework also resolves the apparent contradiction between time-symmetric quantum evolution and irreversible measurement. From within the coherence frame, the system evolves forward in time. But from the observer’s point of view—post-collapse—reconstruction of that evolution can only occur retrospectively. Time symmetry is preserved within the CI; causal irreversibility begins at the CB. What appears paradoxical becomes a matter of frame perspective.
...
\appendix
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[\appendixname~\thesection]{Substrate Variable Definitions and Tick Pacing}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsection[\appendixname~\thesubsection]{Variable Definitions}
...
\subsection[\appendixname~\thesubsection]{Tick Pacing and Gradient Tension (Preliminary)}
...
Zurek, W. H. (2003). Decoherence, einselection, and the quantum origins of the classical. \textit{Reviews of Modern Physics}, 75(3), 715–775. \url{https://doi.org/10.1103/RevModPhys.75.715}
% Reference 
\bibitem{grw-collapse}
Ghirardi, G. C., Rimini, A., and Weber, T. (1986). Unified dynamics for microscopic and macroscopic systems. \textit{Physical Review D}, 34(2), 470–491. \url{https://doi.org/10.1103/PhysRevD.34.470}
% Reference 
\bibitem{price-retrocausality}
